...
**Repository:** `GlacierEQ/glean-enterprise-search-claim-fence`  
**Company lens (independent):** Glean (`glean`)  
**Innovation:** Enterprise Search Claim Fence  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Glean affiliation, proprietary access, or production deployment.
...
Attach ACL-bound evidence digests to every answer claim; drop claims without permitted sources.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Glean employment, endorsement, or proprietary systems access.
...
File: `src/enterprise_search_claim_fence.py`

- Expand `EnterpriseSearchClaimFence` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_enterprise_search_claim_fence.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_enterprise_search_claim_fence.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Glean APIs
