...
def test_contradiction_metadata_cannot_hide_behind_support_stance():
    h=hits(); h[0]["contradicts_units"]=["u2"]; r=evaluate(payload(hits=h)); assert r.decision is Decision.REFUSE; assert r.result["contradicted_units"]==["u2"]; assert "contradicted_claim_units_exceeded" in r.reasons
...
def test_payload_cannot_control_evaluator_clock():
    r=evaluate(payload(now=1000.0)); assert r.decision is Decision.REFUSE; assert "payload_keys_unknown:now" in r.reasons
...
def test_library_boundary_enforces_aggregate_input_size(monkeypatch):
    ...
    r=evaluate(); assert r.decision is Decision.REFUSE; assert "input_too_large" in r.reasons
