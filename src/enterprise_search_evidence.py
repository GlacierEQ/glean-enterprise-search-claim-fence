...
class EnterpriseSearchEvidence:
    ...
    # Caller-supplied policy may tighten these defaults, never weaken them.
    MIN_POLICY_SCORE = 0.6
    MIN_POLICY_STRENGTH = 0.5
    MIN_POLICY_CLAIM_COVERAGE = 1.0
    ...
    @classmethod
    def _policy_unit(cls, value: Any, label: str, *, floor: float) -> float:
        result = cls._unit(value, label)
        if result < floor:
            raise ValueError(f"{label}_below_policy_floor")
        return result
    ...
    def _text_list(
        ...
        *,
        ...
        values = sorted(
            {cls._text(value, f"{label}_{index}") for index, value in enumerate(raw)}
        )
    ...
    def _policy(cls, raw: Any) -> dict[str, Any]:
        ...
        return {
            "min_score": cls._policy_unit(
                raw.get("min_score", cls.MIN_POLICY_SCORE),
                "policy_min_score",
                floor=cls.MIN_POLICY_SCORE,
            ),
            "min_evidence_strength": cls._policy_unit(
                raw.get("min_evidence_strength", cls.MIN_POLICY_STRENGTH),
                "policy_min_evidence_strength",
                floor=cls.MIN_POLICY_STRENGTH,
            ),
            "max_age_seconds": cls._number(
                raw.get("max_age_seconds", 86400.0), "policy_max_age_seconds"
            ),
            "min_supporting_hits": cls._integer(
                raw.get("min_supporting_hits", 2),
                "policy_min_supporting_hits",
                minimum=1,
            ),
            "min_supporting_sources": cls._integer(
                raw.get("min_supporting_sources", 2),
                "policy_min_supporting_sources",
                minimum=1,
            ),
            "min_claim_coverage": cls._policy_unit(
                raw.get("min_claim_coverage", cls.MIN_POLICY_CLAIM_COVERAGE),
                "policy_min_claim_coverage",
                floor=cls.MIN_POLICY_CLAIM_COVERAGE,
            ),
            "max_contradicted_units": cls._integer(
                raw.get("max_contradicted_units", 0),
                "policy_max_contradicted_units",
            ),
        }
    ...
    def _hit(
        ...
        *,
        ...
        if indexed_at > now:
            raise ValueError(f"hit_{index}_indexed_in_future")
        acl_any = cls._text_list(
            raw.get("acl_any", []),
            f"hit_{index}_acl_any",
            limit=cls.MAX_ENTITLEMENTS,
        )
        supports = cls._text_list(
            raw.get("supports_units", []),
            f"hit_{index}_supports_units",
            limit=cls.MAX_UNITS,
        )
        contradicts = cls._text_list(
            raw.get("contradicts_units", []),
            f"hit_{index}_contradicts_units",
            limit=cls.MAX_UNITS,
        )
        unknown_units = (set(supports) | set(contradicts)) - claim_units
        if unknown_units:
            raise ValueError(
                f"hit_{index}_unknown_claim_units:" + ",".join(sorted(unknown_units))
            )
        ...
        return {
            ...
            "document_digest": cls._sha(
                raw.get("document_digest"), f"hit_{index}_document_digest"
            ),
            "score": score,
            ...
            "weighted_evidence": score * strength,
            "supports_units": supports,
            "contradicts_units": contradicts,
        }
    ...
    def evaluate(
        self, req: EnterpriseSearchEvidenceRequest
        ...
        if not request_shape_blocked:
            try:
                ...
                if req.not_after is not None and now > self._number(
                    req.not_after, "not_after"
                ):
                    reasons.append("request_expired")

                actor_id = self._text(payload.get("actor_id"), "actor_id")
                actor_entitlements_list = self._text_list(
                    payload.get("actor_entitlements", []),
                    "actor_entitlements",
                ...
                claim_units_list = self._text_list(
                    payload.get("claim_units"),
                    "claim_units",
                    limit=self.MAX_UNITS,
                    allow_empty=False,
                )
                claim_units = set(claim_units_list)
                snapshot = self._sha(
                    payload.get("index_snapshot_digest"), "index_snapshot_digest"
                )
                ...
                if work_units > budget:
                    ...
                    hits = [
                        self._hit(
                            ...
                            index,
                            now=now,
                            actor_entitlements=actor_entitlements,
                            claim_units=claim_units,
                            policy=policy,
                        )
                        for index, raw in enumerate(hits_raw)
                    ]
                    ...
                    eligible = [hit for hit in hits if hit["eligible"]]
                    supports = [
                        hit for hit in eligible if hit["stance"] == "support"
                    ]
                    supported_units = {
                        unit
                        for hit in supports
                        for unit in hit["supports_units"]
                        if hit["weighted_evidence"] > 0.0
                    }
                    contradicted_units = {
                        unit
                        for hit in eligible
                        for unit in hit["contradicts_units"]
                        if hit["weighted_evidence"] > 0.0
                    }
                    support_sources = {hit["source_uri"] for hit in supports}
                    coverage = len(supported_units) / len(claim_units)
                    ...
                    if coverage < policy["min_claim_coverage"]:
                        reasons.append("insufficient_claim_coverage")
                    if len(contradicted_units) > policy["max_contradicted_units"]:
                        reasons.append("contradicted_claim_units_exceeded")

                    manifest = {
                        "schema": "glaciereq.enterprise-search-evidence.v1",
                        "actor_id": actor_id,
                        "actor_entitlements_digest": _digest(actor_entitlements_list),
                        "query": query,
                        "claim": claim,
                        "claim_units": claim_units_list,
                        "index_snapshot_digest": snapshot,
                        "policy": policy,
                        "ranked_hits": hits,
                    }
                    evidence_digest = _digest(manifest)
                    expected = payload.get("expected_evidence_digest")
                    if expected is not None and self._sha(
                        expected, "expected_evidence_digest"
                    ) != evidence_digest:
                    ...
                    result = {
                        ...
                        "contradicted_units": sorted(contradicted_units),
                        "claim_coverage": coverage,
                        "inaccessible_hit_count": sum(
                            not hit["entitled"] for hit in hits
                        ),
                        "stale_hit_count": sum(not hit["fresh"] for hit in hits),
                        "weak_evidence_hit_count": sum(
                            not hit["strength_ok"] for hit in hits
                        ),
                    }
            except ValueError as exc:
                reasons.append(str(exc))
        ...
        if not reasons:
            reasons = ["claim_supported_by_entitled_fresh_enterprise_evidence"]
        metrics = {
            ...
            "claim_coverage": result.get("claim_coverage", 0.0),
            "inaccessible_hit_count": result.get("inaccessible_hit_count", 0),
            "stale_hit_count": result.get("stale_hit_count", 0),
            "weak_evidence_hit_count": result.get("weak_evidence_hit_count", 0),
        }
        digest = _digest(
            {
        ...
        return EnterpriseSearchEvidenceReceipt(
...
def cli(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Verify enterprise search claim evidence with ACL and freshness checks."
    )
    ...
    try:
        ...
        receipt = EnterpriseSearchEvidence().evaluate(
