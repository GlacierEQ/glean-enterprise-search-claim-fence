"""Enterprise Search Claim Fence — SCAFFOLD STUB.

Company lens: Glean (independent; no affiliation).
Bottleneck: Workplace answers mix permissioned retrieval with uncited model fill.
...
class EnterpriseSearchClaimFenceRequest:
...
@dataclass(frozen=True)
class EnterpriseSearchClaimFenceReceipt:
    decision: Decision
    reasons: tuple[str, ...]
    digest: str
...
class EnterpriseSearchClaimFence:
    """Central mechanism stub.

    Contract the filling AI must preserve:
    - `evaluate(req)` returns a EnterpriseSearchClaimFenceReceipt
    ...
    def evaluate(self, req: EnterpriseSearchClaimFenceRequest) -> EnterpriseSearchClaimFenceReceipt:
        reasons: list[str] = []
        ...
        if reasons:
            ...
            return EnterpriseSearchClaimFenceReceipt(
...
Mechanism = EnterpriseSearchClaimFence
